 ...
 <!-- Form Pencarian -->
    <div class="mb-8 max-w-2xl mx-auto">
        <form method="GET" action="{{ route('products.index') }}" class="relative">
            <div class="flex items-center bg-white rounded-xl shadow-lg border border-gray-200 overflow-hidden">
                <!-- Input Search -->
                <div class="flex-1 relative">
                    <input 
                        ...
                        name="query" 
                        value="{{ request('query') }}" 
                        placeholder="Cari produk berdasarkan nama, deskripsi, atau bahan..." 
                        class="w-full px-4 py-3 pl-12 text-gray-700 bg-transparent focus:outline-none focus:ring-0 text-base"
                        autocomplete="off"
                    >
                    <!-- Search Icon -->
                ...
                <button 
                    type="submit" 
                    class="px-6 py-3 bg-gradient-to-r from-emerald-600 to-teal-600 text-white font-semibold hover:from-teal-500 hover:to-emerald-500 transition-all duration-300 focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:ring-offset-2"
                >
                    Cari
                </button>
                ...
                <a 
                    ...
                    class="px-4 py-3 bg-gray-100 text-gray-600 hover:bg-gray-200 transition-colors duration-300 focus:outline-none focus:ring-2 focus:ring-gray-300 focus:ring-offset-2"
        ...
        <div class="mt-4 text-center">
            <p class="text-gray-600">
                Hasil pencarian untuk: <span class="font-semibold text-gray-800">"{{ request('query') }}"</span>
                @if(isset($products) && $products->count() > 0)
                    <span class="text-emerald-600">({{ $products->count() }} produk ditemukan)</span>
                @else
                    <span class="text-red-500">(Tidak ada produk ditemukan)</span>
                @endif
            </p>
        </div>
        @endif
    </div>
    <div class="grid grid-cols-1 md:grid-cols-3 gap-8 px-4 sm:px-6 lg:px-8 py-8">
        @forelse ($products as $product)
        <div class="bg-white text-gray-900 p-6 rounded-2xl shadow-xl flex flex-col h-full transform transition-all duration-300 hover:scale-[1.02] hover:shadow-2xl">
            <!-- Gambar -->
             <img
            ...
            <table class="min-w-full divide-y divide-gray-200 admin-product-table"> {{-- Tambahkan class untuk styling responsif opsional --}}
                <thead class="bg-gray-100">
                    <tr>
                        ...
                        <th scope="col" class="px-3 py-2 text-left text-xs font-medium text-gray-500 uppercase tracking-wider w-20">Gambar</th>
                        {{-- Kolom Nama Produk dibuat lebih fleksibel --}}
                        <th scope="col" class="px-3 py-2 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Nama Produk - ID - BAHAN</th> {{-- Header digabung --}}
                        ...
                        <th scope="col" class="px-3 py-2 text-left text-xs font-medium text-gray-500 uppercase tracking-wider w-28">Harga</th>
                        {{-- Kolom Stok dibuat sangat ringkas --}}
                        <th scope="col" class="px-3 py-2 text-left text-xs font-medium text-gray-500 uppercase tracking-wider w-16">Stok</th>
                        {{-- Kolom Aksi dibuat ringkas dan rata tengah --}}
                        <th scope="col" class="px-3 py-2 text-center text-xs font-medium text-gray-500 uppercase tracking-wider w-28">Aksi</th>
                    </tr>
                </thead>
                <tbody class="bg-white divide-y divide-gray-200">
                    ...
                    <tr class="hover:bg-gray-50 transition-colors duration-150">
                        ...
                        <td class="px-3 py-2" data-label="Nama Produk - ID - BAHAN:"> {{-- Tambahkan data-label untuk responsif --}}
...
<div class="products-container mx-auto py-10">
    ...
    @forelse ($products as $product)
        ...
        <div class="bg-white text-gray-900 p-6 rounded-2xl shadow-xl flex flex-col h-full transform transition-all duration-300 hover:scale-[1.02] hover:shadow-2xl">
            <img
                ...
                alt="{{ $product->nama_produk }}"
                onclick="openImageModal('{{ $product->gambar ? asset('storage/' . $product->gambar) : 'https://via.placeholder.com/300/CCCCCC/FFFFFF?text=Tidak+Ada+Gambar' }}')">
            <h2 class="text-xl font-bold text-gray-900 mb-2">{{ $product->nama_produk }}</h2>
            <p class="text-base text-gray-700 mb-2">{{ $product->deskripsi }}</p>
            <p class="text-lg font-semibold text-emerald-700 mb-2">Harga Rp{{ number_format($product->harga, 0, ',', '.') }}</p>
            <p class="text-sm text-gray-600 mb-1">Bahan: {{ $product->bahan }}</p>
            <p class="text-sm text-gray-600 mb-1">Ukuran: {{ $product->ukuran }}</p>
            <p class="text-sm text-gray-600 mb-4">Stok: {{ $product->stok }}</p>
    ...
    @empty
        <p class="text-center text-gray-500 col-span-full py-12">
            @if(request('query'))
                Tidak ada produk yang cocok dengan pencarian "{{ request('query') }}".
            @else
